...
import { 
  ...
  AssistantSession, AssistantMessage, AssistantFile, AssistantSenderType,
  SourceType, MessageDirection, Role, AgreementStatus, AgreementSourceType
} from '../types';
import { parseOFWExport, parseGmailExport, parseGenericText, ParsedConversation } from '../utils/parsers';

// --- SYSTEM INSTRUCTIONS ---
// This constant is the Single Source of Truth for the AI Persona.
// It must be passed as the 'systemInstruction' or 'system_prompt' to any LLM used.
...
const handleResponse = async (query: any) => {
  ...
  if (error) {
    ...
    // Return empty array/null instead of throwing to prevent app crash if tables don't exist yet
    return []; 
  }
  return data;
};
...
export const api = {
  ...
  getConversations: async (): Promise<Conversation[]> => {
    // We fetch conversations and their participants in one go
  ...
  getRecentMessages: async (limit = 100): Promise<Message[]> => {
    const { data, error } = await supabase
        .from('messages')
        .select('*')
        .order('sent_at', { ascending: true }) // We want chronological for context
        // We can't use limit easily with ascending if we want "last X", so we sort descending then flip?
        // Let's grab last 200 desc, then flip.
  ...
  createLegalDocument: async (doc: Partial<LegalDocument>): Promise<LegalDocument> => {
    const { data, error } = await supabase.from('legal_documents').insert({
      ...
      notes: doc.notes,
      file_url: doc.fileUrl // Ensure file URL is correctly mapped to DB column
    }).select().single();
  ...
  // Helper to fetch ALL active clauses for context
  ...
  // Helper to fetch ALL active agreement items for context
  ...
  // Save Assistant Message (DB Only)
