...
export interface LegalClause {
  ...
  clauseRef: string; // e.g. "Section 4.1"
  topic: string;
  fullText: string;
  summary?: string;
  isActive: boolean;
...
export interface AssistantMessage {
  ...
  content: string;
  createdAt: string;
  // Linkages to objects created by this message/action
  linkedTargetType?: 'conversation' | 'issue' | 'event' | 'person' | 'legal_document' | 'agreement' | 'none';
  linkedTargetId?: string;
  // If this message represents a file upload
  fileId?: string;
}
...
export interface AssistantFile {
  ...
  sessionId: string;
  fileName: string;
  fileUrl: string; // Mock/Real storage URL
  mimeType: string;
